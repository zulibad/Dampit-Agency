...
class InvoiceController extends Controller
{
    public function index()
    {
        if(Auth::user()->akses=="pelanggan"){
            $invoice  = Invoice::with(['customer'])->where('customer_id', Auth::user()->id)->paginate(10);
        }else{
            $invoice  = Invoice::with(['customer'])->orderBy('created_at', 'DESC')->paginate(10);
        }
    ...
    {
        $customers = User::orderBy('created_at', 'DESC')->where('akses', 'pelanggan')->get();
